...
namespace Zenject.Internal
{
    public static class ZenMenuItems
    {
        ...
        [MenuItem("Assets/Create/Zenject/Reload Scene Contract Config", false, 80)]
        public static void CreateReloadSceneContractConfig()
        {
            var folderPath = ZenUnityEditorUtil.GetCurrentDirectoryAssetPathFromSelection();
            ...
            {
                EditorUtility.DisplayDialog("Error",
                    "ReloadSceneContractConfig objects must be placed directly underneath a folder named 'Resources'.  Please try again.", "Ok");
                return;
            }

            var config = ScriptableObject.CreateInstance<ReloadSceneContractConfig>();

            ZenUnityEditorUtil.SaveScriptableObjectAsset(
                Path.Combine(folderPath, ReloadSceneContractConfig.ResourcePath + ".asset"), config);
        }
