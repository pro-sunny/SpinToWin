...
namespace Zenject.Internal
{
    ...
    public static class SceneParentAutomaticLoader
    {
        ...
        static void OnPlayModeStateChanged(PlayModeStateChange state)
        {
            ...
            {
                var defaultContractsMap = LoadReloadContractsMap();
                var sceneInfos = GetLoadedZenjectSceneInfos();
                foreach (var sceneInfo in sceneInfos)
                {
                    // at this point we only have parent ContractNames, so iterate through it
                    foreach (var contractName in sceneInfo.SceneContext.ContractNames)
                    {
                        LoadDefaultSceneForContract(sceneInfo, contractName, defaultContractsMap);
                    }
                }
            }
        }
            ...
            Dictionary<string, string> defaultContractsMap)
        {
            foreach (var parentContractName in sceneInfo.SceneContext.ParentContractNames)
            {
                ...
                {
                    //at this point we have parent scene already loaded
                    SceneManager.UnloadSceneAsync(sceneInfo.Scene);
                    SetReloadScene(sceneInfo.Scene, parentContractName);
                    
                    // commented this out because sceneInfo.Scene is null at this point (scene was unloaded)
                ...
                // unload child scene to prevent it from being loaded twice
                // during AppEnvironment pipeline
                SceneManager.UnloadSceneAsync(sceneInfo.Scene);
                SetReloadScene(sceneInfo.Scene, parentContractName);
        ...
        private static void SetReloadScene(Scene sceneInfoScene, string parentContractName)
        {
            var configs = Resources.LoadAll<ReloadSceneContractConfig>(ReloadSceneContractConfig.ResourcePath);
            ...
            foreach (var config in configs)
            {
                foreach (var info in config.DefaultContracts)
                {
                    ...
                    if (info.ContractName == parentContractName)
                    {
                        info.Scene = AssetDatabase.LoadAssetAtPath<SceneAsset>(sceneInfoScene.path);
        ...
        static Dictionary<string, string> LoadReloadContractsMap()
        {
            var configs = Resources.LoadAll<ReloadSceneContractConfig>(ReloadSceneContractConfig.ResourcePath);
